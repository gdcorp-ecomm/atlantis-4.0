...
namespace Atlantis.Framework.AddOrderLevelPromoPL.Interface
{
  public class PrivateLabelPromoCurrency
  {
    ...
    /// <returns></returns>
    public static bool IsPromoCurrencyValid(PrivateLabelPromoCurrency promoCurrency, ref PLOrderLevelPromoException exception)
    {
      bool result = true;
      exception = null;
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("The promoCurrency supplied to this method was null", PLOrderLevelPromoExceptionReason.Other);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You did not specifiy a type of award. Available options are \"DollarOff\" and \"PercentOff\"", PLOrderLevelPromoExceptionReason.InvalidOrUnspecifiedAward);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You cannot set an \"AwardValue\" and/or a \"MinSubtotal\" to zero.", PLOrderLevelPromoExceptionReason.InvalidOrUnspecifiedAward);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You cannot allocate more than 100% off of an order. Please check the 'AwardValue' field.", PLOrderLevelPromoExceptionReason.AwardValueGreaterThanAllowed);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You cannot allocate a greater monetary discount than the minimum order subtotal. Please check the 'AwardValue' and 'MinSubtotal' fields.", PLOrderLevelPromoExceptionReason.AwardValueGreaterThanAllowed);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You cannot specify an invalid currency type. Please check the 'CurrencyType' field.", PLOrderLevelPromoExceptionReason.InvalidCurrencySpecification);
      }
      ...
      {
        result = false;
        exception = new PLOrderLevelPromoException("You cannot specify a discount amount greater than the system will allow. Specified=[" + promoCurrency.AwardValue + "], MaxAllowable=[" + promoCurrency.MaxAwardValuePercentOff + "]", PLOrderLevelPromoExceptionReason.InvalidOrUnspecifiedAward);
      }
      ...
      else if (promoCurrency.TypeOfAward == AwardType.DollarOff)
      {
        ...
        {
          result = false;
          exception = new PLOrderLevelPromoException("You cannot specify a discount amount greater than the system will allow. Specified (as a %)=[" + actualPctOff + "], MaxAllowable=[" + promoCurrency.MaxAwardValuePercentOff + "]", PLOrderLevelPromoExceptionReason.InvalidOrUnspecifiedAward);
        }
      }
