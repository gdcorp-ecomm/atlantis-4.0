    ...
    public IResponseData RequestHandler(RequestData oRequestData, ConfigElement oConfig)
    {
      ...
      if (String.IsNullOrEmpty(((FastballGetOffersMsgDataRequestData)oRequestData).ISC))
      {
        try
        {
          ...
          int count = 0;
          var offersResponse = offersWs.GetOffersAndMessageData(requestData.ChannelRequestXml,
                                                                requestData.CandidateRequestXml);
          if (offersResponse.ResultCode != 0 || offersResponse.SelectedOffers == null)
          {
            throw new Exception(
              string.Format(
                "GetOffersAndMessageData call failed. Status:{0}, ChanneRequestlXml:{1}, CandidateRequestXml:{2}",
                offersResponse.ResultCode, requestData.ChannelRequestXml, requestData.CandidateRequestXml));
          }
