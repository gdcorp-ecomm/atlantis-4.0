...
{
  [DataContract]
  public class QSCAddPackageResponseData : IResponseData
  {
        private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCAddPackageResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCAddPackageResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCAddPackageResponseData(responseDetail response)
    {
