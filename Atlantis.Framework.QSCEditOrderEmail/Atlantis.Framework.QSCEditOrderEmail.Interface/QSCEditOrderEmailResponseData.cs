...
{
  [DataContract]
  public class QSCEditOrderEmailResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCEditOrderEmailResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCEditOrderEmailResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCEditOrderEmailResponseData(responseDetail response)
    {
