...
{
  [DataContract]
  public class QSCRemovePackageResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCRemovePackageResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCRemovePackageResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCRemovePackageResponseData(responseDetail response)
    {
