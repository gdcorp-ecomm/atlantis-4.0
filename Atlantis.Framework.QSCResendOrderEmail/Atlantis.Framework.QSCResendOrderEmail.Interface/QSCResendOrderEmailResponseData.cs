...
{
  [DataContract]
  public class QSCResendOrderEmailResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCResendOrderEmailResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCResendOrderEmailResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCResendOrderEmailResponseData(responseDetail response)
    {
