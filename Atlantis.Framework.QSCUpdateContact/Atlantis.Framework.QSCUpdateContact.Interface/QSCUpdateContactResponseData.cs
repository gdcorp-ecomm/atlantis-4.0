...
{
  [DataContract]
  public class QSCUpdateContactResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCUpdateContactResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCUpdateContactResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCUpdateContactResponseData(responseDetail response)
    {
