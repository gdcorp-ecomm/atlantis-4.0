...
{
  [DataContract]
  public class QSCUpdateOrderNotesResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;
    
    public QSCUpdateOrderNotesResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCUpdateOrderNotesResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCUpdateOrderNotesResponseData(responseDetail response)
    {
