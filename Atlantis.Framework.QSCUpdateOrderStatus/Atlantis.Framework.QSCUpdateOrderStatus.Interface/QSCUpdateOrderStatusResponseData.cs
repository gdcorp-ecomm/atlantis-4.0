...
{
  [DataContract]
  public class QSCUpdateOrderStatusResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCUpdateOrderStatusResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCUpdateOrderStatusResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCUpdateOrderStatusResponseData(responseDetail response)
    {
