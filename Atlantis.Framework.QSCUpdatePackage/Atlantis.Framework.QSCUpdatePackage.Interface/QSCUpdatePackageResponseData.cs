...
{
  [DataContract]
  public class QSCUpdatePackageResponseData : IResponseData
  {
    private readonly AtlantisException _ex;
    private responseDetail response;

    public QSCUpdatePackageResponseData(RequestData request, Exception ex)
    {
      _ex = new AtlantisException(request, ex.Source, ex.Message, ex.StackTrace, ex);
    }

    public QSCUpdatePackageResponseData(AtlantisException aex)
    {
      _ex = aex;
    }

    public QSCUpdatePackageResponseData(responseDetail response)
    {
