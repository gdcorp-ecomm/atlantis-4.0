  ...
  internal class PackageBuilder
  {
    ...
    public static IEnumerable<IDomainRegistrationProductPackageGroup> PackageDataToDomainRegistrationProduct(IList<PackageData> packageDataItemGroups, IProviderContainer container)
    {
      ...
      foreach (var packageDataItemGroup in packageDataItemGroups)
      {
        ...
        domainRegProductPackageGroup.InPreRegPhase = packageDataItemGroup.InLaunchPhase;

        if (packageDataItemGroup.InLaunchPhase)
        {
          foreach (var key in packageDataItemGroup.LaunchPhasePackages.Keys)
          {
            var launchPhaseDomainProductPackage = new DomainProductPackage(container) {Domain = domainRegProductPackageGroup.Domain};
            ...
            domainRegProductPackageGroup.PreRegPhasePackages.Add(key, launchPhaseDomainProductPackage);
          }
        }
        else
        {
          var registrationPackage = new DomainProductPackage(container) {Domain = domainRegProductPackageGroup.Domain};
    ...
    public static IEnumerable<PackageData> DomainRegistrationProductToPackageData(IEnumerable<IDomainRegistrationProductPackageGroup> domainRegistrationProductPackageGroup)
    {
      ...
      foreach (var domainGroup in domainRegistrationProductPackageGroup)
      {
        ...
        {
          InLaunchPhase = domainGroup.InPreRegPhase,
        ...
        if (domainGroup.InPreRegPhase)
        {
          packageData.LaunchPhasePackages = new Dictionary<LaunchPhases, List<IDictionary<string, object>>>(domainGroup.PreRegPhasePackages.Count);

          foreach (var launchPhasePackage in domainGroup.PreRegPhasePackages)
          {
