...
{
  public class EmailFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderDotTypeFormField(DotTypeFormFieldTypes fieldType, IProviderContainer providerContainer, out IList<IFormField> formFields)
    {
      ...
      try
      {
        var field = providerContainer.GetData<IDotTypeFormsField>(FieldTypeDataKeyConstants.EMAIL_DATA_KEY, null);
        if (field != null)
        {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileRichDatetimeFieldTypeHandler";
    ...
    private static IList<IFormField> ConvertToFormFields(IDotTypeFormsField field)
    {
      ...
      var formField = new FormField { Name = field.FieldName, LabelText = field.FieldLabel, Type = FormFieldTypes.InputEmail };
