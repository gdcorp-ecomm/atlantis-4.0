...
{
  public class MobileDefaultCheckboxFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileDefaultCheckboxFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<label>" + HttpUtility.HtmlEncode(field.FieldLabel) + "</label>");
      result.Append("</div>");
