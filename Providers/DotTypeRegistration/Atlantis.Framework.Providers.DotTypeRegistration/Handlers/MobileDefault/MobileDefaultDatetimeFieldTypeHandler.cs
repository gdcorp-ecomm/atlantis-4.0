...
{
  public class MobileDefaultDatetimeFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      try
      {
        var field = providerContainer.GetData<IDotTypeFormsField>(FieldTypeDataKeyConstants.DATETIME_DATA_KEY, null);
        if (field != null)
        {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileDefaultDatetimeFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<div><input type='datetime' class='max rnd' name='" + field.FieldName + "'>" + "</input></div>");
      result.Append("</div>");
