...
{
  public class MobileDefaultEmailFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileDefaultEmailFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<div><input type='email' class='max rnd' name='" + field.FieldName + "'>" + "</input></div>");
      result.Append("</div>");
