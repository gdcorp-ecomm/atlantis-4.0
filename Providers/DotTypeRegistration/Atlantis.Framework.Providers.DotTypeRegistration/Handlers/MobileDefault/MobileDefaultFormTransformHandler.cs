...
namespace Atlantis.Framework.Providers.DotTypeRegistration.Handlers.MobileDefault
{
  public class MobileDefaultFormTransformHandler :  IDotTypeFormTransformHandler
  {
                                    ...
                                    out Dictionary<string, string> formSchemasHtml)
    {
        ...
        if (formSchema.Form != null)
        {
          ...
          foreach (var domain in domains)
          {
            ...
            foreach (var field in fields)
            {
              ...
              {
                var exception = new AtlantisException("MobileDefaultFormTransformHandler.TransformFormToHtml", "0", "Invalid field type", field.FieldName, null, null);
      ...
      {
        var exception = new AtlantisException("MobileDefaultFormTransformHandler.TransformFormToHtml", "0", ex.Message, formSchema.ToString(), null, null);
        Engine.Engine.LogAtlantisException(exception);
        throw;
      }
