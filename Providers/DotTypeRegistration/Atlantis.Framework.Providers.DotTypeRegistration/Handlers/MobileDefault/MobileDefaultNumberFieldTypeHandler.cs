...
{
  public class MobileDefaultNumberFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileDefaultNumberFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<div><input type='number' class='max rnd' name='" + field.FieldName + "'>" + "</input></div>");
      result.Append("</div>");
