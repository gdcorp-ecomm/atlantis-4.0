...
{
  public class MobileDefaultPhoneFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileDefaultPhoneFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<div><input type='phone' class='max rnd' name='" + field.FieldName + "'>" + "</input></div>");
      result.Append("</div>");
