...
{
  public class MobileRichStringFieldTypeHandler : IDotTypeFormFieldTypeHandler
  {
    public bool RenderField(FormFieldTypes fieldType, IProviderContainer providerContainer, out string htmlData)
    {
      ...
      catch (Exception ex)
      {
        ...
        const string SOURCE = "RenderField - MobileRichStringFieldTypeHandler";
    ...
    private static string ConvertToHtml(IDotTypeFormsField field)
    {
      ...
      result.Append("<input type='text' class='max rnd' name='" + field.FieldName + "'>" + "</input>");
      result.Append("</div>");
