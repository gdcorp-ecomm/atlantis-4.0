...
{
  internal class QaPsPhraseHandler : ILanguagePhraseHandler
  {
    ...
    {
      string pseudoText = String.Empty;
      string paddedParagraph = String.Empty;

      pseudoText = currentText.Replace('a', 'å').Replace('e', 'é').Replace('i', 'î').Replace('o', 'ò').Replace('u', 'ü').Replace('A', 'Å').Replace('E', 'É').Replace('I', 'Î').Replace('O', 'Ò').Replace('U', 'Ü');
      ...
      if (pseudoLength > 0)
      {
        ...
        if (pseudoLines.Length > 1)
        {
          ...
          {
            paddedParagraph += opening_char + pseudoText + extendedString + closing_char + Environment.NewLine;
          }
        }
        else
        {
          paddedParagraph += opening_char + pseudoText + extendedString + closing_char;
        }
      }
      return paddedParagraph;
    }

    private string GenerateExtension(int extensionLength)//removing pseudo variable as this will only process qa-ps files, a different handler will process qa-pz files
    {
