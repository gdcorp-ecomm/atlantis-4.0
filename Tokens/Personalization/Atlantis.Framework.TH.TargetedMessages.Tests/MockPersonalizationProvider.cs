    ...
    public Personalization.Interface.TargetedMessages GetTargetedMessages(string appId, string interactionPoint)
    {
      ...
      try
      {
        ...
        if (targetMessageXml.Root != null)
        {
          ...
          {
            XmlSerializer serializer = new XmlSerializer(typeof(Personalization.Interface.TargetedMessages), targetMessageXml.Root.GetDefaultNamespace().NamespaceName);
            messages = (Personalization.Interface.TargetedMessages)serializer.Deserialize(reader);
            reader.Close();
          }
        }
      }
